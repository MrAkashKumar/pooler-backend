...
public interface PbChatThreadRepository extends JpaRepository<PbChatThreadEntity, Long> {
    ...
    @Query("SELECT c FROM PbChatThreadEntity c WHERE c.expiresAt < :now AND c.status = :status")
    List<PbChatThreadEntity> findExpiredThreads(@Param("now") Instant now, @Param("status") ChatThreadStatus status);
